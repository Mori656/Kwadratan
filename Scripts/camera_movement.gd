...
@export var min_zoom: Vector2 = Vector2(1.8,1.8)
...
@export var vieport_size: Vector2 = Vector2(320,180)
...
func _process(delta: float) -> void:
	...
	elif mouse_position.x >= vieport_size.x - edge_margin:
		move_vector.x = camera_speed * delta
		
	if mouse_position.y <= edge_margin:
		move_vector.y = -camera_speed * delta
	elif mouse_position.y >= vieport_size.y - edge_margin:
		move_vector.y = camera_speed * delta
	
	position += move_vector
	#Utrzymywanie kamery w polu gry
	position.x = clamp(position.x, vieport_size.x / zoom.x, map_size.x - vieport_size.x / zoom.x)
	position.y = clamp(position.y, vieport_size.y / zoom.y, map_size.y - vieport_size.y / zoom.y)
